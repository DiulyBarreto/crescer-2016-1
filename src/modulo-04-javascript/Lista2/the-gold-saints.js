...
// arredondar número com duas casas decimais
Number.prototype.arredondarNumero = function() {
  return Math.round(this * 100) / 100;
};

Number.prototype.converterCmParaMetros = function() {
  return this/100;
};
...
function obterCavaleiroComMaisGolpes() {
  ...
  for(var i = 1; i < goldSaints.length; i++) {
    if(goldSaints[i].golpes.length > maisGolpes.golpes.length)
...
function obterAlturaMedia() {
  ...
  return ((alturaMedia = alturaMedia/goldSaints.length).converterCmParaMetros()).arredondarNumero();
};
...
function obterAlturaMediana() {
  ...
  var mediana = ((arrayAux[meio] + arrayAux[meio - 1])/2).converterCmParaMetros();
  return mediana.arredondarNumero();
};
...
function obterPesoMedio() {
  ...
  for(var i = 0; i < goldSaints.length; i++) {
    if(apenasComPesoDefinido(goldSaints[i])) {
  ...
  return (pesoMedio.lbParaKilos()).arredondarNumero();
};
...
function obterPesoMedioDoadores() {
  ...
  for(var i = 0; i < doadores.length; i++) {
    if(apenasComPesoDefinido(doadores[i])){
...
function obterIMC() {
  ...
  for(var i = 0; i < goldSaints.length; i++) {
    if(apenasComPesoDefinido(goldSaints[i])){
      var peso = goldSaints[i].pesoLb.lbParaKilos();
      var altura = goldSaints[i].alturaCm.converterCmParaMetros();
      massaCorporal.push((peso / Math.pow(altura, 2)).arredondarNumero());
    }
  }
