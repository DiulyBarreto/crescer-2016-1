...
namespace Repositorio
{
    public class RepositorioFuncionarios
    {
        ...
        {
            var funcionariosFiltrados = Funcionarios.Where(funcionario => funcionario.Cargo.Equals(cargo)).ToList();
            return funcionariosFiltrados;
        }

        public IList<Funcionario> OrdenadosPorCargo()
        {
            var ordenadosPorCargo = Funcionarios.OrderBy(funcionario => funcionario.Cargo.Titulo).ThenBy(funcionario => funcionario.Nome).ToList();
            return  ordenadosPorCargo;
        }

        public IList<Funcionario> BuscarPorNome(string nome)
        {
            var funcionariosFiltrados = Funcionarios.Where(funcionario => funcionario.Nome.IndexOf(nome, StringComparison.OrdinalIgnoreCase) >= 0).ToList();
            return funcionariosFiltrados;
        }        
        ...
        public IList<Funcionario> BuscarPorTurno(params TurnoTrabalho[] turnos)
        {
            ...
            {
                funcionariosPorTurno = Funcionarios.Where(funcionario => funcionario.TurnoTrabalho.Equals(turno)).ToList();
            }
        ...
        public IList<Funcionario> AniversariantesDoMes()
        {
            ...
            var aniversariantes = Funcionarios.Where(funcionario => funcionario.DataNascimento.Month == mesAtual).ToList();
